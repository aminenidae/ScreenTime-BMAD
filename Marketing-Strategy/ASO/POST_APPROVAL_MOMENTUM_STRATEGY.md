...
3. **Prompt for ratings early — but only inside parent mode.** 75%+ users leave by day 2, so the prompt must fire at an early delight moment. BUT: prompting on the child's device is a net-negative: kids can leave 1-star revenge reviews as payback for screen-time restrictions, under-13 child Apple IDs may have In-App Ratings disabled (prompt fires, submits nothing, slot burned), and Kids-Category/COPPA concerns apply. The rating prompt is gated behind `SessionManager.isParentAuthenticated` + foregrounded main app. Trigger architecture (Option B, 2026-04-17): per-trigger flags with `firstParentSuccess` (earnedMinutes > 0 on dashboard) and `firstWeeklyWin` (currentStreak >= 3); stronger trigger preempts weaker; Apple's 3-per-365-day cap is the ultimate guardrail. See `docs/rating-prompt-wiring-resume.md` for call sites and verification steps.
...
4. **Verify the rating-prompt wiring on TestFlight.** The prompt is gated behind parent-mode authentication with two triggers (`firstParentSuccess` at earnedMinutes > 0, `firstWeeklyWin` at currentStreak >= 3). Verification steps: fresh install → parent onboarding → dashboard view meets a trigger → expect system prompt + `DEBUG_LOG_RATING_PROMPT_FIRED`. Also verify child-device shield drops do NOT fire the prompt, and that a locked parent session logs `parent_not_authenticated`. Full matrix in `docs/rating-prompt-wiring-resume.md`. `firstStreakMilestone` (streak >= 7) is a deferred third trigger pending Day 14 data.
...
- **v1.5 (2026-04-17):** Rating-prompt architecture revised after a code audit: child-device `.firstUnlock` trigger removed (kid-retaliation 1-stars, under-13 submit failures, Kids-Category/COPPA concerns). Prompt now gated behind `SessionManager.isParentAuthenticated`. Moved from single global flag to per-trigger flags (Option B): `firstParentSuccess` (earnedMinutes > 0) and `firstWeeklyWin` (currentStreak >= 3); stronger trigger preempts weaker; Apple's 3/365 cap is the guardrail. `firstStreakMilestone` deferred. Code commits `eda8a0f` + `b6c545a` on `feature/streamline-usage-recording`. Full wiring: `docs/rating-prompt-wiring-resume.md`.
